...
// html과 처리 시점이완전 다르니까...
	...
	List<BoardMasterVO> boardList = //유지의문제 - DB를 경유해야한다 ->servlet
			(List<BoardMasterVO>)request.getAttribute("boardList");//sql문을 넘겨넘겨 받아서 가지고옴
	int size = 0;
	if(boardList!=null){
		size = boardList.size();
	}
...
<%
		...
		for(int i=nowPage*numPerPage;i<(nowPage*numPerPage)+numPerPage;i++){
			if(size == i) break;
			BoardMasterVO bmVO = boardList.get(i);// 데이터 꺼내는 반복문/ 데이터 껀수만큼
%>	      
        	<tr>
        		<td><%=bmVO.getB_no()%></td>
...
<a href="javascript:boardDetail('<%=bmVO.getB_no()%>')" style="text-decoration:none;color:#000000">        		
        		...
        		<td>
<%
	if(bmVO.getB_file()!=null){ 
%>        		        		
        		<a href="javascript:fileDown('<%=bmVO.getB_file() %>')">
        		<%=bmVO.getB_file()%>
        		</a>
<%
	}
