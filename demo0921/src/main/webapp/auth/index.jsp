	...
	// easyui_common.jsp
		...
		function receiveMemoList(){
			$("#dg_receiveMemoList").datagrid({
				...
				,onSelect: function(index, row){// 선택한 번호를 담는 용도의 onSelect
					gm_no = row.NO;// 데이터 그리드 선택시 해당 로우의 아이디 담기
				}
			});	
			$("#d_member").hide();
			// after
		...
		function memoContent(){
			...
			$("#dlg_memoContent").dialog({
				...
				// 이 기능을 구현하는데 있어서 컨트롤 계층의 메소드부터 호출이 되어야 하는 건지??
				// 아니면 로직클래스에 메소드 호출만으로도 충분한지를 고민해 보기 -> 객체주입관계와 메소드 선언을 결정함.
				// 설계자는 기능 담당자가 구현해야하는 페이지이름과 메소드, 그리고 파라미터와 리턴타입 모두를
				// 다 정해놓고 담당자는 메소드안에 기능구현만 하도록 할 것.
		...
		function memoContentClose(){
			// receiveMemoList();
			// Single Page App은 과연 옳은 선택일까??
			location.href="/auth/index.jsp";//URL이 변경->기존의 요청이 끊어지고 새로운 요청을 처리한다 - 세션이나 쿠기가 바뀐다.
		    ...
		    <table id="dg_receiveMemoList" class="easyui-datagrid" style="width:700px;height:250px"
		        ...
		        <thead>
		            <tr>
		                ...
		                <th data-options="field:'MEM_NAME',width:300,align:'left'">보낸사람 이름</th>
